...
namespace CapaVista
{
    public partial class MDIPrincipal : Form
    {
        ...
        private void ingresoFacturaDeComprasToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (seguridad.PermisosAcceso("303", txtusuario.Text) == 1)
            {
                bit.user(txtusuario.Text);
                bit.insert("Ingreso A Facturacion de Compras", 303);
                Procesos.Facturacion.frmIngresoFacturacion mantenimiento = new Procesos.Facturacion.frmIngresoFacturacion();
                mantenimiento.MdiParent = this;
                mantenimiento.Show();
            ...
            {
                bit.user(txtusuario.Text);
                bit.insert("Trato de Ingresar A Facturacion de Compras", 303);
                MessageBox.Show("El Usuario No Cuenta Con Permisos De Acceso A La Aplicación");
            }
        }
    }
